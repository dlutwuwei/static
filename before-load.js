...
const init = (global = window, baseFontSize = 100) => {
  ...
  (function () {
            ...
            (s = ua.match (/(msie |trident\/)([\d.]+)/))
              ...
              : // 火狐
                (s = ua.match (/firefox\/([\d.]+)/))
                  ? ((Sys.firefox = s[1]), (Sys.browser = 'firefox'), addClass (
                      'firefox'
                    ))
                  : // 谷歌
                    (s = ua.match (/chrome\/([\d.]+)/))
                      ? ((Sys.chrome = s[1]), (Sys.browser =
                          'chrome'), addClass ('chrome'))
                      : // 欧朋
                        (s = ua.match (/opera.([\d.]+)/))
                          ? ((Sys.opera = s[1]), (Sys.browser =
                              'opera'), addClass ('opera'))
                          : // 苹果
                            (s = ua.match (/version\/([\d.]+).*safari/))
                              ? ((Sys.safari = s[1]), (Sys.browser =
                                  'safari'), addClass ('safari'))
                              : !1;
    // vivo手机特殊化
    ...
    (s = ua.match (/windows.* ([\d.]+)\;/))
      ...
      : (s = ua.match (/mac.*x ([\d.]+)\;/))
          ...
          : (s = ua.match (/iphone.* ([\d]+)\_([\d])/))
              ? ((Sys.ios = s[2] ? (s[1] + s[2]) / 10 : s[1]), (Sys.facility =
                  'iphone'), addClass ('ios'))
              : (s = ua.match (/ipad.* ([\d]+)\_([\d])\_/))
                  ? ((Sys.ios = s[2]
                      ? (s[1] + s[2]) / 10
                      : s[1]), (Sys.facility = 'ipad'), addClass ('ios'))
                  : (s = ua.match (/android ([\d.]+)\;/))
                      ? ((Sys.android = s[1]), (Sys.facility =
    ...
    (s = ua.match (/(android|iphone|ios|ipad|mobile|silk-accelerated=true)/))
      ? (Sys.mobile = !0)
      : !1;
    Sys.telPrefix = Sys.facility === 'android' && parseInt (Sys.android, 10) < 4
      ? 'tel://'
      : 'tel:';
    global.sysType = Sys;
